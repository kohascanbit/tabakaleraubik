        ...
        if ($urldocumentbibliofile) {
            ...
            unless ($isItem) {
                # if (!$record->field('856')) {
                    ...
                    $newField->add_subfields('x' => $input->param('rightuse'));
    ...
    if ($op ne 'delete') {
        for (my $i=0; $i < @$fieldsForm; $i++) {
            ...
            if ($fileName) {
                ...
                # Restricciones
                my $folderdocument = 'documents/';
    			if (($query->param('rightuse') eq 'CR') or ($query->param('rightuse') eq 'TAB')) {
    				$folderdocument = 'documents_private/';
    			}
                $uploadDirFin .= ($fileName =~ /^.+\.(gif|jpg|jpeg|png)$/i)?'covers/':$folderdocument;
                mkdir $uploadDirFin unless (-d $uploadDirFin);
                
	            if ($fileName2 =~ /-/){
	            	 $fileName = $fileName2 . $extension;
	            } else{
	            	my @files = glob($uploadDirFin . $fileName2 . '*');
	                my $num_doc_max = 0;
	                
	                for (@files) {
		                my $name = $_;
		                my $num_doc = GetNumDoc($name);
		                if ($num_doc > $num_doc_max){
		                	$num_doc_max = $num_doc;
		                }
		            }
	            	$num_doc_max = $num_doc_max + 1;
	            	if ($num_doc_max > 1){
	            		$fileName = $fileName2 . "-" . $num_doc_max . $extension;
	            	} else{
	            		$fileName = $fileName2 . $extension;
	            	}
	            }
...
sub showDocLR
{
    ...
    if ($dir && -d $dir) {
        ...
        my @files = ();
        if ($biblionumber =~ /_/){
        	@files = glob($dir2 . $biblionumber . '*');
        } else{
        	@files = glob($dir2 . $biblionumber . '*');
        }
        if (@files) {
            for (@files) {
                $_ =~ /\/$biblionumber\.[a-z0-9]{2,4}$/i;
                $_ =~ s/^.+\///;
                push (@urldocuments,  $url2 . $_);
                $existingdocument = 1;
            }
        }
        
        if (($input->param('rightuse') eq '') or ($input->param('rightuse') eq 'CR') or ($input->param('rightuse') eq 'TAB')) {
	        $url2 = $url;
	        $dir2 = $dir;
	        $dir2 .= '/' unless ($dir =~ /\/$/);
	        $dir2 .= 'documents_private/';
	        $url2 .= '/' unless ($url2 =~ /\/$/);
	        $url2 .= 'documents_private/';
	        my @files = ();
	        if ($biblionumber =~ /_/){
	        	@files = glob($dir2 . $biblionumber . '*');
	        } else{
         ...
         $template->param(urldocument => \@urldocuments) if ($template);
    }
