...
sub searchResults {
    ...
    my $isFilmoteca;
    if ($search_context eq 'opacFilmoteca') {
        $search_context = 'opac';
        $isFilmoteca = 'true';
    }
    my $dbh = C4::Context->dbh;
...
# For each grouping of items (onloan, available, unavailable), we build a key to store relevant info about that item
	...
	my $interface = $search_context eq 'opac' ? 'OPAC' : '';
        if ($isFilmoteca eq 'true'){
         if (!$scan && C4::Context->preference($interface . "XSLTResultsDisplayFilmoteca")) {
            $oldbiblio->{XSLTResultsRecord} = XSLTParse4Display($oldbiblio->{biblionumber}, $marcrecord, $interface."XSLTResultsDisplayFilmoteca", 1, \@hiddenitems);
            # the last parameter tells Koha to clean up the problematic ampersand entities that Zebra outputs
        }
        }
	elsif (!$scan && C4::Context->preference($interface . "XSLTResultsDisplay")) {
