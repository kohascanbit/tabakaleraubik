...
sub create_input {
    ...
    "<input type=\"text\"
        ...
        onclick=\"openAuth(this.parentNode.parentNode.parentNode.id,this.parentNode.getElementsByTagName('input')[1].id,'".$tagslib->{$tag}->{$subfield}->{authtypecode}."','auth'); return false;\" tabindex=\"1\" title=\"Tag Editor\">...</a>
    ";
    # it's a plugin field
    }
...
my $myindexTag = $input->param('indexTag');
...
$template->param(nonav   => $nonav,index=>$myindex,indexTag=>$myindexTag,authtypecode=>$authtypecode,);
...
#------------------------------------------------------------------------------------------------------------------------------
    ...
    if (!$duplicateauthid or $confirm_not_duplicate) {
        ...
        if ($myindex) {
            print $input->redirect("blinddetail-biblio-search.pl?authid=$authid&index=$myindex&indexTag=$myindexTag");
        } else {
