...
my ( $template, $borrowernumber, $cookie ) = get_template_and_user(
    {
        template_name   => "opac-detail-filmoteca.tmpl",
        query           => $query,
        type            => "opac",
...
if (C4::Context->preference("OPACXSLTDetailsDisplayFilmoteca") ) {
    $template->param( 'XSLTBloc' => XSLTParse4Display($biblionumber, $record, "OPACXSLTDetailsDisplayFilmoteca" ) );
}
...
if ($session->param('busc')) {
    ...
    if ($paging{'previous'}->{biblionumber}) {
        $previous = 'opac-detail-filmoteca.pl?biblionumber=' . $paging{'previous'}->{biblionumber};
    ...
    if ($paging{'next'}->{biblionumber}) {
        $next = 'opac-detail-filmoteca.pl?biblionumber=' . $paging{'next'}->{biblionumber};
    ...
    for (my $j = 0; $j < @arrBiblios; $j++) {
        ...
        push @listResults, {index => $j + 1 + $offset, biblionumber => $arrBiblios[$j], title => ($arrBiblios[$j] == $biblionumber)?'':$dataBiblioPaging->{title}, author => ($arrBiblios[$j] != $biblionumber && $dataBiblioPaging->{author})?$dataBiblioPaging->{author}:'', url => ($arrBiblios[$j] == $biblionumber)?'':'opac-detail-filmoteca.pl?biblionumber=' . $arrBiblios[$j]};
    }
...
foreach my $subscription (@subscriptions) {
    ...
    $cell{recievedlist} = $subscription->{recievedlist};
    push @subs, \%cell;
}
...
if ( C4::Context->preference( "SocialNetworks" ) ) {
    $template->param( current_url => C4::Context->preference('OPACBaseURL') . "/cgi-bin/koha/opac-detail-filmoteca.pl?biblionumber=$biblionumber" );
