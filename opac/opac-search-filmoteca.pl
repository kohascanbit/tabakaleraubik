...
if ($format =~ /(rss|atom|opensearchdescription)/) {
    $template_name = 'opac-opensearch-filmoteca.tmpl';
}
elsif (@params && $build_grouped_results) {
    $template_name = 'opac-results-grouped-filmoteca.tmpl';
}
elsif ((@params>=1) || ($cgi->param("q")) || ($cgi->param('multibranchlimit')) || ($cgi->param('limit-yr')) ) {
    $template_name = 'opac-results-filmoteca.tmpl';
}
else {
    $template_name = 'opac-advsearch-filmoteca.tmpl';
...
if ($template_name eq 'opac-results-filmoteca.tmpl') {
    ...
    if ($server && $server =~/biblioserver/) { # this is the local bibliographic server
        ...
        if ($build_grouped_results) {
            foreach my $group (@{ $results_hashref->{$server}->{"GROUPS"} }) {
                ...
                my @group_results = searchResults( 'opacFilmoteca', $query_desc, $group->{'group_count'},$results_per_page, 0, $scan,
                                                   $group->{"RECORDS"});
                push @newresults, { group_label => $group->{'group_label'}, GROUP_RESULTS => \@group_results };
            }
        } else {
            @newresults = searchResults('opacFilmoteca', $query_desc, $hits, $results_per_page, $offset, $scan,
        ...
        && $format ne 'opensearchdescription' && $format ne 'atom') {
            ...
            if (C4::Context->preference('BiblioDefaultView') eq 'isbd') {
                print $cgi->redirect("/cgi-bin/koha/opac-ISBDdetail-filmoteca.pl?biblionumber=$biblionumber");
            } elsif  (C4::Context->preference('BiblioDefaultView') eq 'marc') {
                print $cgi->redirect("/cgi-bin/koha/opac-MARCdetail-filmoteca.pl?biblionumber=$biblionumber");
            } else {
                print $cgi->redirect("/cgi-bin/koha/opac-detail-filmoteca.pl?biblionumber=$biblionumber");
            } 
            exit;
        }
        if ($hits) {
