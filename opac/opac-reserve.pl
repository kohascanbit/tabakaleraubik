...
foreach my $biblioNumber (@biblionumbers) {
    ...
    # LEIRE Reservar analíticas
    my $recordType = GetNormalizedRecordType( $marcrecord);
	my $biblionumberOrigin = 0;
	if ($recordType eq 'AR'){
		$biblionumberOrigin = GetBiblionumberOrigin($biblioNumber);
		if ($biblionumberOrigin > 0){
			$biblioNumber = $biblionumberOrigin;
...
foreach my $biblioNum (@biblionumbers) {
    ...
    # LEIRE Reservar analíticas
    my $recordType = GetNormalizedRecordType( $record);
	my $biblionumberOrigin = 0;
	if ($recordType eq 'AR'){
		$biblionumberOrigin = GetBiblionumberOrigin($biblioNum);
		if ($biblionumberOrigin > 0){
			$biblioNum = $biblionumberOrigin;
			$record = GetMarcBiblio($biblioNum);
		}
	}
    ...
    my $recordType = GetNormalizedRecordType( $record);
	$template->param(recordType => $recordType);
	...
	if (($reservedfor eq $borrowernumber) && ($recordType ne 'SE')){
