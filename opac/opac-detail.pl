...
$template->param(
	...
	biblionumberOrigin => $biblionumberOrigin
);
...
# Leire
my @suggestionList = GetSuggestionList($biblionumber);
if (scalar(@suggestionList) >= 1) {
	$template->param(Suggestions => 1);
	$template->param(SUGGESTION_LIST => @suggestionList);
} else{
	$template->param(Suggestions => 0);
}
