...
my ( $template, $borrowernumber, $cookie ) = get_template_and_user(
    {
        template_name   => "aqua-detail-eu.tmpl",
        query           => $query,
        type            => "opac",
...
if ($session->param('busc')) {
    ...
    if ($paging{'previous'}->{biblionumber}) {
        $previous = 'aqua-detail-es.pl?biblionumber=' . $paging{'previous'}->{biblionumber};
    ...
    if ($paging{'next'}->{biblionumber}) {
        $next = 'aqua-detail-es.pl?biblionumber=' . $paging{'next'}->{biblionumber};
    ...
    for (my $j = 0; $j < @arrBiblios; $j++) {
        ...
        push @listResults, {index => $j + 1 + $offset, biblionumber => $arrBiblios[$j], title => ($arrBiblios[$j] == $biblionumber)?'':$dataBiblioPaging->{title}, author => ($arrBiblios[$j] != $biblionumber && $dataBiblioPaging->{author})?$dataBiblioPaging->{author}:'', url => ($arrBiblios[$j] == $biblionumber)?'':'aqua-detail-es.pl?biblionumber=' . $arrBiblios[$j]};
    }
...
# NUEVO PARA AQUABROWSER
$template->param( 'AquaBrowserRoot' => C4::Context->preference('AquaBrowserRoot') );
# FIN NUEVO PARA AQUABROWSER
