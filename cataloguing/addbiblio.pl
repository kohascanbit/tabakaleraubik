        ...
        $subfield_data{marc_value} =
            "<input type=\"text\"
                    ...
                    <span class=\"subfield_controls\"><a href=\"#\" class=\"buttonDot\"
                       onclick=\"openAuth(this.parentNode.parentNode.parentNode.id,this.parentNode.parentNode.getElementsByTagName('input')[1].id,'".$tagslib->{$tag}->{$subfield}->{authtypecode}."','biblio'); return false;\" tabindex=\"1\" title=\"Tag Editor\"><img src=\"/intranet-tmpl/prog/img/edit-tag.png\" alt=\"Tag Editor\" /></a></span>
            ";
    # it's a plugin field
    }
...
my $erabil_eskubide_loop = C4::Koha::GetAuthorisedValues( 'ERABILESKUBIDE' );
...
$template->param(
	erabil_eskubide_loop => $erabil_eskubide_loop
);
...
#-------------------------------------------------------------------------------------
    ...
    if ($showFilesLocalRepository) {
        ...
        deleteFileLR($dirFLR . 'documents/', 'R_' . $biblionumber);
    }
