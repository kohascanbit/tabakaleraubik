...
function Clic$function_name(i) {
 	...
 	var tag_000 = findChildWithIdLike(document.body, 'tag_000_subfield').value;
 	var mat = tag_000.charAt('6') + tag_000.charAt('7');
	defaultvalue=mat + document.getElementById(\"$field_number\").value;
...
sub plugin {
    ...
    if ($result eq ''){
    	...
    	my $materialType =  substr $result, 0, 2;
    	my $result2 = substr $result, 2;
    	$result = $materialType . "$dateentered" . "b        xxu||||| |||| 00| 0 $lang d" unless $result2;
    }
    my $errorXml = '';
